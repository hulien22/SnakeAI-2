...
int main(int argc, char** argv) {
    ...
    while (f < 9000) {
        ...
        for (int j=0; j<POPULATION_SIZE; ++j) {
            ...
            // std::vector<double> in = {1,0,1,-1,-0.5,1,0,1,-1,-0.5,1,0,1,-1,-0.5,1,0,1,-1,-0.5,1,0,1,-1,-0.5};
            std::vector<double> in = {1,0,1,-1};
            std::vector<double> outputs(NUMBER_OF_OUTPUTS);
            std::vector<double> w = genomes[j].getWeights();
            // for (size_t q=0; q<w.size(); ++q){
            //     std::cout << w[q] << ", ";
            // }
            // std::cout << std::endl;
            nnet.SetWeights(w);
            outputs = nnet.Calculate(in);
            genomes[j].setFitness(outputs[0] + outputs[1] - outputs[2]);
        ...
        std::vector<Genome> oldg;
        for (size_t q=0; q<genomes.size(); ++q){
            oldg.emplace_back(genomes[q].getWeights(), genomes[q].getFitness());
        }
        ...
        bool check = true;
        for (size_t q=0; q<genomes.size(); ++q){
            if(check && oldg[q].getFitness() == genomes[q].getFitness()) check = true;
            else check = false;
        }
